<template>
  <div class="home" v-if="repoName">
    ...
    <ul class="main-points">
      ...
      <li>
        created
        <router-link to="/projects/" class="info" v-bind:href="postLink">these</router-link> projects
      </li>
      <li>
        pushed last commit to
...
export default {
  ...
  data: function() {
    return {
      ...
      linkedinProfileLink:
        "https://www.linkedin.com/in/przemys%C5%82aw-bielak-052343122/",
  ...
  methods: {
    ...
    updateGithubInfo: function() {
        ...
        if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
          ...
          for (var event = 0; event < contentJSON.length; event++) {
            if (contentJSON[event].type == "PushEvent") {
              var PayloadMail = contentJSON[event].payload.commits
                .slice(-1)
                .pop().author.email;
              if (PayloadMail === this.email) {
                ...
                this.commitLink =
                  "https://github.com/" +
                  contentJSON[event].repo.name +
                  "/commit/" +
                  contentJSON[event].payload.head;
              }
              break;
            }
          }
      ...
      xmlHttp.open(
        "GET",
        "https://api.github.com/users/przemekBielak/events/public",
        true
      );
      xmlHttp.send(null);
    }
